...
func update_stats():
	...
	wave_label.text = tr("Wave %s") % stats.current_wave
	kills_label.text = tr("Kills: %s") % stats.total_kills
	points_label.text = tr("Points: %s") % stats.total_points
	var time_seconds = int(stats.play_time)
	var minutes = time_seconds / 60
	var seconds = time_seconds % 60
	time_label.text = tr("Time: %s") % [minutes, seconds]
	
	record_wave_label.text = tr("Wave %s") % int(best.wave)
	record_kills_label.text = tr("Kills: %s") % int(best.kills)
	record_points_label.text = tr("PPoints: %s") % int(best.points)
